...
$cekAwal = $db->QUERY("SELECT * FROM tb_kriteria ORDER BY kode ASC");
//$x = 0;
while   ($cekAtribut = mysqli_fetch_array($cekAwal)){
    //$x++;
    if($cekAtribut['atribut'] == 'benefit'){$ambil[$cekAtribut['kode']] = 'MAX';}else{$ambil[$cekAtribut['kode']] = 'MIN';}
}
//var_dump($ambil);
...
<?php 
		$a    = $db->QUERY ("SELECT MAX(k1) as k1max, MAX(k2) as k2max, MAX(k3) as k3max, MAX(k4) as k4max, MAX(k5) as k5max, 
																 MAX(k6) as k6max, MAX(k7) as k7max, MAX(k8) as k8max,MAX(k9) as k9max, MAX(k10) as k10max, 
                                 MIN(k1) as k1min, MIN(k2) as k2min, MIN(k3) as k3min, MIN(k4) as k4min, MIN(k5) as k5min, 
																 MIN(k6) as k6min, MIN(k7) as k7min, MIN(k8) as k8min,MIN(k9) as k9min, MIN(k10) as k10min 
																 FROM tb_alternatif WHERE kode !='A0' ");
		$a1 = mysqli_fetch_array($a);
		if($ambil['K1'] == 'MAX'){$k1 = $a1['k1max'];}else{$k1 = $a1['k1min'];}
    if($ambil['K2'] == 'MAX'){$k2 = $a1['k2max'];}else{$k2 = $a1['k2min'];}
    if($ambil['K3'] == 'MAX'){$k3 = $a1['k3max'];}else{$k3 = $a1['k3min'];}
    if($ambil['K4'] == 'MAX'){$k4 = $a1['k4max'];}else{$k4 = $a1['k4min'];}
    if($ambil['K5'] == 'MAX'){$k5 = $a1['k5max'];}else{$k5 = $a1['k5min'];}
    if($ambil['K6'] == 'MAX'){$k6 = $a1['k6max'];}else{$k6 = $a1['k6min'];}
    if($ambil['K7'] == 'MAX'){$k7 = $a1['k7max'];}else{$k7 = $a1['k7min'];}
    if($ambil['K8'] == 'MAX'){$k8 = $a1['k8max'];}else{$k8 = $a1['k8min'];}
    if($ambil['K9'] == 'MAX'){$k9 = $a1['k9max'];}else{$k9 = $a1['k9min'];}
    if($ambil['K10'] == 'MAX'){$k10 = $a1['k10max'];}else{$k10 = $a1['k10min'];}
      ...
      //Cari Cost
      $sqlCost = $db->QUERY("SELECT * FROM tb_alternatif");
      WHILE($cariCost = mysqli_fetch_array($sqlCost)){
        if($cariCost['k1'] == 0){$cost1 = 0;}else{$cost1 = round(1/$cariCost['k1'],3);}
        if($cariCost['k2'] == 0){$cost2 = 0;}else{$cost2 = round(1/$cariCost['k2'],3);}
        if($cariCost['k3'] == 0){$cost3 = 0;}else{$cost3 = round(1/$cariCost['k3'],3);}
        if($cariCost['k4'] == 0){$cost4 = 0;}else{$cost4 = round(1/$cariCost['k4'],3);}
        if($cariCost['k5'] == 0){$cost5 = 0;}else{$cost5 = round(1/$cariCost['k5'],3);}
        if($cariCost['k6'] == 0){$cost6 = 0;}else{$cost6 = round(1/$cariCost['k6'],3);}
        if($cariCost['k7'] == 0){$cost7 = 0;}else{$cost7 = round(1/$cariCost['k7'],3);}
        if($cariCost['k8'] == 0){$cost8 = 0;}else{$cost8 = round(1/$cariCost['k8'],3);}
        if($cariCost['k9'] == 0){$cost9 = 0;}else{$cost9 = round(1/$cariCost['k9'],3);}
        if($cariCost['k10'] == 0){$cost10 = 0;}else{$cost10 = round(1/$cariCost['k10'],3);}
        $kodeCost = $cariCost['kode'];
        $sqlUpdateCost = $db->QUERY("UPDATE `tb_alternatif` SET `cost1`='$cost1',`cost2`='$cost2',`cost3`='$cost3',`cost4`='$cost4',
                                                                `cost5`='$cost5',`cost6`='$cost6',`cost7`='$cost7',`cost8`='$cost8',
                                                                `cost9`='$cost9',`cost10`='$cost10' WHERE `kode` = '$kodeCost' ");
      }
        ...
        //COST
				...
				//$am100  = round(1/$am1['k9'],3); 
				//SUM  kolom
				$aSUM    = $db->QUERY ("SELECT SUM(k1) as k1, SUM(k2) as k2, SUM(k3) as k3, SUM(k4) as k4, SUM(k5) as k5, 
                                        SUM(k6) as k6, SUM(k7) as k7, SUM(k8) as k8,SUM(k9) as k9, SUM(k10) as k10,
                                        SUM(cost1) as k1cost, SUM(cost2) as k2cost, SUM(cost3) as k3cost, SUM(cost4) as k4cost, SUM(cost5) as k5cost, 
                                        SUM(cost6) as k6cost, SUM(cost7) as k7cost, SUM(cost8) as k8cost,SUM(cost9) as k9cost, SUM(cost10) as k10cost
																 FROM tb_alternatif ");
				$tSum = mysqli_fetch_array($aSUM);
        if($ambil['K1'] == 'MAX'){$n1 =  round($am11/$tSum['k1'],3);}else{if($tSum['k1cost'] == 0){$n1 = 0;}else{$n1 = round($am21/$tSum['k1cost'],3);}}
        if($ambil['K2'] == 'MAX'){$n2 =  round($am12/$tSum['k2'],3);}else{if($tSum['k2cost'] == 0){$n2 = 0;}else{$n2 = round($am21/$tSum['k2cost'],3);}}
        if($ambil['K3'] == 'MAX'){$n3 =  round($am13/$tSum['k3'],3);}else{if($tSum['k3cost'] == 0){$n3 = 0;}else{$n3 = round($am21/$tSum['k3cost'],3);}}
        if($ambil['K4'] == 'MAX'){$n4 =  round($am14/$tSum['k4'],3);}else{if($tSum['k4cost'] == 0){$n4 = 0;}else{$n4 = round($am21/$tSum['k4cost'],3);}}
        if($ambil['K5'] == 'MAX'){$n5 =  round($am15/$tSum['k5'],3);}else{if($tSum['k5cost'] == 0){$n5 = 0;}else{$n5 = round($am21/$tSum['k5cost'],3);}}
        if($ambil['K6'] == 'MAX'){$n6 =  round($am16/$tSum['k6'],3);}else{if($tSum['k6cost'] == 0){$n6 = 0;}else{$n6 = round($am21/$tSum['k6cost'],3);}}
        if($ambil['K7'] == 'MAX'){$n7 =  round($am17/$tSum['k7'],3);}else{if($tSum['k7cost'] == 0){$n7 = 0;}else{$n7 = round($am21/$tSum['k7cost'],3);}}
        if($ambil['K8'] == 'MAX'){$n8 =  round($am18/$tSum['k8'],3);}else{if($tSum['k8cost'] == 0){$n8 = 0;}else{$n8 = round($am21/$tSum['k8cost'],3);}}
        if($ambil['K9'] == 'MAX'){$n9 =  round($am19/$tSum['k9'],3);}else{if($tSum['k9cost'] == 0){$n9 = 0;}else{$n9 = round($am21/$tSum['k9cost'],3);}}
        if($ambil['K10'] == 'MAX'){$n10 =  round($am110/$tSum['k10'],3);}else{if($tSum['k10cost'] == 0){$n10 = 0;}else{$n10 = round($am21/$tSum['k10cost'],3);}}

					//hitung
					$kode = $iSS['kode'];
					$sql    = $db->QUERY ("INSERT INTO `tb_normalisasi`(`kode`, `k1`, `k2`, `k3`, `k4`, `k5`, `k6`, `k7`, `k8`, `k9`, `k10`) 
																						  VALUES ('$kode','$n1','$n2','$n3','$n4','$n5','$n6','$n7','$n8','$n9','$n10') ");	
				}
				//Update Cost k4 dan k5
				//SUM K4 dan K5
			$s    = $db->QUERY ("SELECT MAX(k1) as k1, MAX(k2) as k2, MAX(k3) as k3, MAX(k4) as k4, MAX(k5) as k5, 
																 MAX(k6) as k6, MAX(k7) as k7, MAX(k8) as k8,MIN(k9) as k9, MIN(k10) as k10  
																 FROM tb_normalisasi  ");
      ...
      <tbody>
        ...
        ?>
          <tr>
            ...
            <td class="text-center"><?= $tampil['kode']; ?></td>
            <td class="text-left"><?= $tampil['alternatif']; ?></td>
            <td class="text-center"><?= $tampil['k1']; ?></td>
            ...
            <td class="text-center table-danger"><?= $tampil['k9']; ?></td>
            <td class="text-center table-danger"><?= $tampil['k10']; ?></td>
          </tr>
    ...
    <table class="table table-striped">
      ...
      <tbody>
        ...
        ?>
          <tr>
            <th class="text-center"><?= $no++ ?>.</th>
            <td class="text-center"><?php echo $bb1;?></td>
            ...
            <td class="text-center table-danger"><?php echo $bb9;?></td>
            <td class="text-center table-danger"><?php echo $bb10;?></td>
...
<?php?>		  
     ...
     <h2>4. Data Normalisasi Bobot</h2>
    <table class="table table-striped">
      <thead class="table-primary">
        <tr>
          <th class="text-center">#</th>
